...
//Noi 2 mang so nguyen a, b tra ve mang ket qua
int* concateArr(int *a, int n, int *b, int m, int &size) {
	int*c = new int[n + m];
	size = n + m;

	//Chep cac phan tu cua a qua c
	for (int i = 0; i < n; i++) {
		*(c + i) = *(a + i);
	}

	//Chep cac phan tu cua b qua c
	for (int i = 0; i < m; i++) {
		*(c + i + n) = *(b + i);
	}
...
//Sap xep
void sortArr(int *a, int n, bool func(int, int) = cmpMin) {
	if (a == NULL)
		return;
...
//Them phan tu vao mang (noi dan kich thuoc)
void pushBackArr(int *&a, int &n, int x) {
	n = (a == NULL) ? 1 : n + 1;
	int *newArr = new int[n];

	//Chuyen du lieu tu vung nho cu sang vung nho moi
	for (int i = 0; i < n - 1; i++) {
		*(newArr + i) = *(a + i);
	}
